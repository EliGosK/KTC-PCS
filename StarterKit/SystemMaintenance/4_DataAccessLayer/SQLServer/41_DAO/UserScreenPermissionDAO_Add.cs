...
namespace SystemMaintenance.SQLServer.DAO
{
    partial class UserScreenPermissionDAO
    {
        public DataTable LoadUserPermissionJoinScreen(Database database, string I_SCREEN_CD)
        {
            ...
            sb.AppendLine("   ISNULL(USR_SCR_PERM.FLG_VIEW, 2) FLG_VIEW,");
            sb.AppendLine("   ISNULL(USR_SCR_PERM.FLG_ADD, 2) FLG_ADD,");
            sb.AppendLine("   ISNULL(USR_SCR_PERM.FLG_CHG, 2) FLG_CHG,");
            sb.AppendLine("   ISNULL(USR_SCR_PERM.FLG_DEL, 2) FLG_DEL,");
            ...
            sb.AppendLine("      select USR_SCR_PERM.* ");
            sb.AppendLine("      from TZ_USER_SCREEN_PERMISSION_MS USR_SCR_PERM");
            sb.AppendLine("      INNER JOIN TZ_SCREEN_MS sc ");
            sb.AppendLine("         ON usr_scr_perm.SCREEN_CD = sc.SCREEN_CD and sc.MAIN_PERMISSION_SCREEN_CD is null");
            sb.AppendLine("      where USR_SCR_PERM.SCREEN_CD = :I_SCREEN_CD ");
            sb.AppendLine("      union all ");
            sb.AppendLine("      select USR_SCR_PERM.* ");
            sb.AppendLine("      from TZ_USER_SCREEN_PERMISSION_MS USR_SCR_PERM");
            sb.AppendLine("      INNER JOIN TZ_SCREEN_MS sc ");
            sb.AppendLine("         ON usr_scr_perm.SCREEN_CD = sc.MAIN_PERMISSION_SCREEN_CD and sc.MAIN_PERMISSION_SCREEN_CD is not null ");
            sb.AppendLine("      where sc.SCREEN_CD = :I_SCREEN_CD");
            sb.AppendLine("   ) usr_scr_perm ");
