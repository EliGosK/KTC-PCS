...
namespace EVOFramework.Windows.Forms
{
	...
	/// </summary>
	public class DateTextBox : System.Windows.Forms.MaskedTextBox, IReadOnly {
		/// <summary>
		...
		// Option       
		private DateTime m_dtMinDateTime = new DateTime(1753, 1, 1, 0, 0, 0, 0);
		private DateTime m_dtMaxDateTime = new DateTime(9998, 12, 31, 23, 59, 59, 999);
		...
		public string Format {
			...
			set {
                ...
                {
                    this.m_strFormat = strTempFormat;
                    this.Mask = "00/00/0000 00:00:00.000";
                }
                ...
                // 20081118 Modify by ASSI.Teerayut
                else if (strTempFormat == MONTH + "/" + DAY + "/" + YEAR)
                {
                    this.m_strFormat = strTempFormat;
                    this.Mask = "00/00/0000";
                }                
                // 20081118 End Modify by ASSI.Teerayut
                else
                {
		...
		private void CheckTextFormat(ref string strText, ref object dtDateTime) {
			try {
				...
				string [] strFormat = this.m_strFormat.Split('/', ' ', ':','.');
				string [] strData = strText.Split('/', ' ', ':', '.');
                ...
                //kim add 20071202 to prevent year minimum
                ...
                else if (this.m_strFormat == MONTH + "/" + DAY + "/" + YEAR)
                {
                    strText = Convert.ToString(iMonth).PadLeft(2, '0')
                    + "/" + Convert.ToString(iDay).PadLeft(2, '0')
                    + "/" + Convert.ToString(iYear).PadLeft(4, '0');
                }
		...
		private void CheckDateFormat(ref string strText, ref object dtDateTime) {
			try {
                ...
                {
                    strText = Convert.ToString(dtDataInput.Month).PadLeft(2, '0')
                    + "/" + Convert.ToString(dtDataInput.Day).PadLeft(2, '0')
                    + "/" + Convert.ToString(dtDataInput.Year).PadLeft(4, '0');
